...
describe('evaluateLogFile', () => {
  it('should return an empty object', async () => {
    const { evaluateLogFile } = await import('./index');
    const result = await evaluateLogFile('');
    expect(result).toEqual({});
  });

  it('should return an object with a single key-value pair', async () => {
    const { evaluateLogFile } = await import('./index');
...
2007-04-05T22:12 69.4`;
    const result = await evaluateLogFile(logFile);
    expect(result).toEqual({ 'temp-1': 'precise' });
  });

  it('should return an object with multiple key-value pairs', async () => {
    const { evaluateLogFile } = await import('./index');
    const logFile = `reference 70.0 45.0 6
thermometer temp-1
2007-04-05T22:00 72.4
